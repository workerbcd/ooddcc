...
class Textures(Base):
    def __init__(self, split="test", trf=None, trf_type="test", trf_idSet="imagenet", model="resnet"):
        ...
        self.data_dir = os.path.join(DATA_ROOT, "dtd/images")
        assert os.path.exists(self.data_dir), "Please download the Textures dataset.{}".format(self.data_dir)
...
class ImageNetO(Base):
    def __init__(self, split="train", trf=None, trf_type="test", trf_idSet="imagenet", model="resnet"):
        ...
        # dataset
        root_path = os.path.join(DATA_ROOT, "imagenet-o")
        self.dataset = dset.ImageFolder(
            root=root_path,
            transform=trf
        )
