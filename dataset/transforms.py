...
def get_imgNet_transforms(mode="test", index=None):
    ...
    transform_imgNet_test = transforms.Compose(test_transforms)
...
def get_imgNet_transforms_vit(mode="test", index=None):
    # The chosen B-16_imagenet21 operates on a different resolution. 
    # See: https://github.com/lukemelas/PyTorch-Pretrained-ViT
    if index==None:
        test_trans = [transforms.Resize((384, 384)),
        transforms.ToTensor(),
        transforms.Normalize(0.5, 0.5),
        ]
    else:
        test_trans = [transforms.Resize((384, 384)), aug_type[index],
         transforms.ToTensor(),
         transforms.Normalize(0.5, 0.5),
         ]
...
def get_transforms(dataset="cifar100", mode="test", resize_and_crop=False, model="resnet", index=None):
    ...
    if "cifar" in dataset:
        ...
        if ("vit" in model and "clip" not in model):
            return get_imgNet_transforms_vit(mode, index=index)
        else:
            return get_imgNet_transforms(mode, index=index)
    else:
