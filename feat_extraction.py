...
if "clip" in args.arch:
    ...
    id_train_loader, id_test_loader, ood_loaders = get_loaders_for_ood(args, trf=transform_test_largescale)
else:
...
for split, in_loader in [('train', id_train_loader), ('test', id_test_loader), ]:
    # break
    # for ssplit, in_loader in [('test', id_test_loader)]:
                ...
                for batch_idx, (inputs, targets) in enumerate(in_loader):
                    ...
                    if args.arch!='swin_b':
                        if args.large_scale:
                            ...
                            feat = torch.cat(
                                [F.adaptive_avg_pool2d(layer_feat, 1).squeeze() for layer_feat in feature_list], dim=1)
                    else:
                        feat = feature_list[-1]
            ...
            for batch_idx, (inputs, _) in enumerate(out_loader):
                ...
                if args.arch!='swin_b':
                    if args.large_scale:
                        ...
                        feat = torch.cat([F.adaptive_avg_pool2d(layer_feat, 1).squeeze() for layer_feat in feature_list],
                                         dim=1)
                else:
                    feat = feature_list[-1]
