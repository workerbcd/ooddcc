"""
Resnet adopted from knn-ood:
https://github.com/deeplearning-wisc/knn-ood/blob/master/models/resnet.py
"""
...
class ResNet(AbstractResNet):
    ...
    def forward(self, x, layer_index=None):
        if layer_index is not None:
            return self.intermediate_forward(x, layer_index=layer_index)
        else:
            x = self.features(x)
            x = self.avgpool(x)
            feats = x.view(x.size(0), -1)
...
if __name__ == "__main__":
    ...
    print(net)
    # net = resnet18(pretrained = False, num_classes=1000)
    fake_input = torch.randn((2, 3, 224, 224))
    # net(fake_input,layer_index="all")
    logits, feat_list = net.feature_list(fake_input)
