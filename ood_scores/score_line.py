...
class Line(ScorerBaseFeature):
    ...
    def _cal_center(self):
        if 'resnet50' in self.args.arch:
            arch = 'resnet50'
        elif self.args.arch == 'WRN':
            arch = 'WideResNet'
        else:
            arch = self.args.arch
        load_file = f'./featCache/{self.args.id_dset}_{arch}_taylor_mean_class.npy'
    ...
    def cal_score(self, feats):
        ...
        for feat in tqdm(feats):
            ...
            feat = feat.clip(max=self.clip_threshold)
            logit_clip = self._forward_line(feat)
            score = torch.logsumexp(logit_clip,dim=-1)
            scores.append(_to_np(score))
