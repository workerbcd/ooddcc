...
class FeatStaticBase(ScorerBaseFeature):
    ...
    def fit(self):
        ...
        # == scatter matrices
        self.scatter_within, self.scatter_between, self.scatter_total = self._fit_scatter_mats(self.id_feats,
                                                                                               self.id_labels,
                                                                                               self.center,
                                                                                               self.center_cls)
    ...
    def _fit_scatter_mats(self, id_feats, id_labels, center, center_cls):
        ...
        print(np.isnan(_to_np(center)).any())
    ...
    def plot_stats(self):
        ...
        sns.distplot(_to_np(feat_norm), hist=True, ax=ax, color='b', label="Feature norm distribution", bins=100,
                     kde_kws={})

        # scatterness histogram
        plt.figure()
        ax = plt.gca()
        sns.distplot(_to_np(self.scatterness), hist=True, ax=ax, color='b', label="Scatterness histogram", bins=20,
                     kde_kws={})
...
class Maha(FeatStaticBase):
    ...
    def debug(self):
        ...
        print(
            f"THe rank of scatter dirs with scatteredness above th {th}: {torch.linalg.matrix_rank(self.scatter_dirs[above_th_flags, :])}")
...
class WDiscOOD(DiscBase):
    ...
    def _preprocess_samples_num(self, feats, labels, num):
        new_feats = []
        new_labels = []
        print('cutting samples')
        for i in tqdm(range(self.num_class)):
            class_idx = np.where(labels == i)[0]
            assert len(class_idx)>=num
            class_feats = feats[class_idx[:num], :]
            class_labels = labels[class_idx[:num]]
            new_feats.append(class_feats)
            new_labels.append(class_labels)
        return np.concatenate(new_feats,axis=0), np.concatenate(new_labels, axis=-1)

    def fit(self):
        if self.args.ablation_num is not None:
            print(f"ablation num is {self.args.ablation_num}")
            num_per_class = self.args.ablation_num
            self.id_feats, self.id_labels = self._preprocess_samples_num(self.id_feats, self.id_labels, num_per_class)
        # preprocess feature before fitting
        if self.args.feat_norm:
            print("Normalizing the training features...")
        ...
        self.scatter_within, self.scatter_between, self.scatter_total = self._fit_scatter_mats(feats_processed,
                                                                                               self.id_labels,
                                                                                               self.center,
                                                                                               self.center_cls)
    ...
    def _cal_dist(self, feat, option="ClsEucl", cls_centers=None, center=None, cls_precision=None,
