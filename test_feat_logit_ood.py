...
from utils.utils import print_measures, load_features, get_feat_dims, _to_np, _to_tensor, select_features
...
id_train_feat, id_train_logit, id_train_label = load_features(args, id=True, split="train", last_idx=last_start_idx, \
    featdim=featdims[-1], projdim=proj_dim, feat_space=0)
# if args.score == "Neco" and 'resnet50' in args.arch:
#     id_train_feat, id_train_label, id_train_logit = select_features(id_train_feat, id_train_logit, id_train_label,
#                                                                     0.8)
...
if args.score in ["VIM", "Residual"]:
    ...
    if "resnet" in args.arch or ("vit" in args.arch and "clip" not in args.arch):
        ...
        w = _to_np(params[0])
        b = _to_np(params[1])
    elif "swin" in args.arch or args.arch=='deit':
        params = [p for p in net.head.parameters()]
        w = _to_np(params[0])
        b = _to_np(params[1])
    else:
...
aurocs, fprs = [], []
ood_names = [NAMES[name] for name in args.ood_dsets]
