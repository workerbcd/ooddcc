...
class Evaluator():
    ...
    def eval(self, net, loader, scorer:ScorerBase, in_dist=False):
        ...
        if not in_dist and self.ood_num is not None:
            if len(loader.dataset) > self.ood_num and self.args.id_dset=='imagenet':
        ...
        if in_dist:
            print("The in-distribution data: {}".format(self.args.id_dset))
        ...
        # If OOD, get the evaluation result
            ...
            try:
                ...
                print(f"{loader.dataset.root} OOD Detection Results")
    ...
    def eval_maha_odin(self, loader, scorer:Mahalanobis, in_dist=False):
        ...
        # if ID, get the classification error rate, and save the results
        if in_dist:
            print("The in-distribution data: {}. OOD eval complete".format(self.args.id_dset))
            self.in_scores = scores
        # If OOD, get the evaluation result
        else:
            # save
